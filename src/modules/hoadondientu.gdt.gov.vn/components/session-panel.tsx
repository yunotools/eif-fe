    ...
    <Card className="border-[var(--accent)]">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
        <div className="min-w-0">
          ...
          <h2 className="mt-1 text-xl font-bold">{session.username || "Đã xác thực"}</h2>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="mr-1 rounded-xl border border-[var(--border)] bg-[var(--surface-2)] px-4 py-2 text-right">
            <div className="text-[10px] font-bold uppercase tracking-wider text-[var(--muted)]">Còn lại</div>
            <div className="text-lg font-black tabular-nums text-[var(--accent)]">{countdown.text}</div>
          </div>
          <Button
            type="button"
            variant="secondary"
          ...
          <Button type="button" onClick={() => setShowRefresh((value) => !value)}>
            Làm mới phiên
          </Button>
      ...
      <div className="mt-5 border-t border-[var(--border)] pt-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            <div className="text-[11px] font-bold uppercase tracking-wider text-[var(--muted)]">EIF Session ID</div>
            <code className="mt-1 block break-all text-xs">
              {showSessionId ? session.id : maskedSessionId(session.id)}
            </code>
            <p className="mt-1 text-[11px] text-[var(--muted)]">
              Đây chính là Session ID dùng ở mục “Dùng session có sẵn”. Hãy coi nó như thông tin đăng nhập và không chia sẻ công khai.
            </p>
          </div>
          <div className="flex shrink-0 gap-2">
            <Button type="button" variant="ghost" onClick={() => setShowSessionId((value) => !value)}>
              {showSessionId ? "Ẩn" : "Hiện"}
            </Button>
            <Button type="button" variant="secondary" onClick={() => void copySessionId().catch(() => undefined)}>
      ...
      {syncError ? <div className="mt-4"><ErrorNotice value={syncError} tone="warning" /></div> : null}
      {showRefresh ? <SessionRefreshForm onDone={() => setShowRefresh(false)} /> : null}
    </Card>
  );
}
