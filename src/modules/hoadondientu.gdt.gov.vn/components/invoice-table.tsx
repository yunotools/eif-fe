...
import { numericValue, processingLabel, statusLabel, textValue } from "@modules/hoadondientu.gdt.gov.vn/utils/invoice";

function rowKey(record: InvoiceRecord, index: number): string {
  return String(record.id ?? record.mhdon ?? `${record.khhdon ?? "invoice"}-${record.shdon ?? index}-${index}`);
}
        ...
        <tbody>
          {datas.map((record, index) => (
            <tr key={rowKey(record, index)}>
              ...
              <td>{record.tdlap ? formatDateTime(String(record.tdlap)) : "-"}</td>
              ...
              <td className="whitespace-nowrap font-semibold">{formatMoney(numericValue(record, "tgtttbso"))}</td>
